...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 13-01 T1 | 01 | 1 | LIFE-04 | T-13-01/02 | kind-branch routes only to known consumer RPCs | Deno | `deno test --allow-none supabase/functions/process-jobs/handlers_test.ts` (RED pre-fix) | ✅ extend | ⬜ pending |
...
- [ ] `supabase/tests/life03_standby_notify.sql` — NEW: match_autowithdraw_user_conflicts flips queue to standby + dispatches standby_bumped (LIFE-03, plan 13-02 Task 4).
