...
- **D-03 (DOOR-2 PROD RE-CHECK — DONE 2026-06-03 via Supabase MCP on `ufufmcpnysvwtutpbian`):** `create_blank_itinerary` **CONFIRMED EXISTS on prod** (count=1) → Door 2 works on prod; do NOT rebuild the blank-itinerary RPC. Build the canvas publish-CTA + creator controls on top. Reconcile §2A canvas work with the `open-city` `CreateFlow.tsx` scaffold (separate unmerged branch) — do not double-edit; treat open-city as a known parallel surface.
- **D-03b (TARGETING COLUMNS — NET NEW, verified via prod MCP):** `date_instances.target_genders` / `target_age_range` / `search_radius_km` do NOT exist on prod (count=0) → E11's per-date targeting requires a NEW additive migration to add these columns (matches the date-settings spec: "designed, not built"). `reject_candidate` also absent on prod (E12 = real build). All Phase-3 migrations are LOCAL-only; prod apply GATED. (Phase-2 RPCs confirmed absent on prod too = correctly gated.)
...
### Resolved by research (locked 2026-06-03)
- **D-09 (E12 decline):** Add a new `passed_by_host` value to the `queue_status` enum (additive — own migration, separate from use per PG ADD VALUE tx rule). `reject_candidate` sets the queue_entry to `passed_by_host`, silent removal (candidate not notified). Candidate self-withdraw already exists (`match-withdraw`) — Phase 3 surfaces it + the host withdraw of an outstanding offer.
- **D-10 (E11 field homes):** Creator fields `pay_setting`/`why_note`/`vibe_tags`/`cover_image_url` live on `itineraries` (forked at post time), NOT `date_instances`. `update_itinerary_stops` already sets why_note+cover but NOT pay_setting+vibe_tags → extend it. Targeting cols (`target_genders`/`target_age_range`/`search_radius_km`) are net-new on `date_instances` (one additive migration). Cover UPLOADER is net-new (reuse `apps/web/lib/after5/photos.ts` storage pattern; `CoverPicker` today only re-picks stop photos).
- **D-11 (deferrals):** Per-stop regenerate has NO seam in generate-plan → DEFER to P3 (drop D-02). Reach preview → DEFER to Phase 4 (depends on the targeting/filter data layer).
- **D-12 (E13 no migration):** The lock/offer party can ALREADY read the `date_instances` row post-offer (`date_instances_select_offer_recipient`) + the forked itinerary by id (legacy `itineraries_readable_by_id`) — so stops load via the RLS client with NO new RPC. Reuse `StopRow` + `normalizeNightDetailStops` (extract a shared `PlanTimeline` from `apps/web/app/feed/NightDetailSheet.tsx`). Do NOT reuse `get_night_detail` (blind/pre-swipe only). Also delete the dead `host.bio` branch in OfferDetail (F#5).
- **D-13 (E14 audit not rebuild):** Server-runtime offer email ALREADY built + working (`/api/offers/notify-offered` runtime=nodejs → `sendOfferReceivedEmail` → Resend); in-app `offer_received` dispatched transactionally inside the offer RPC (the reliability guarantee); push fans out via `/api/cron/push-web` (inert without VAPID). E14 = CHAIN AUDIT + deep-link guarantee + execution-time verify that RESEND_API_KEY is set in Vercel server runtime, NOT a rebuild.
- **Path corrections:** interested list is `apps/web/app/dates/[slug]/interested/` (not `[instanceId]`); `NightDetailSheet.tsx` is under `apps/web/app/feed/` (not components/).
