// match-withdraw/index.test.ts — validation + arg-shaping (no idem_key).
...
Deno.test('match-withdraw: missing instance -> 400', async () => {
  setEnv();
  resetStub();
  assertEquals((await handler(post({}))).status, 400);
});

Deno.test('match-withdraw: shapes p_actor/p_instance only', async () => {
  setEnv();
  const stub = resetStub({ user: { id: 'actor-1' } });
  await handler(post({ instance: 'i-1' }));
  assertEquals(stub.calls[0].name, 'match_withdraw');
  assertEquals(stub.calls[0].args, { p_actor: 'actor-1', p_instance: 'i-1' });
});
