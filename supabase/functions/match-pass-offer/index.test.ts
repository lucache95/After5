// match-pass-offer/index.test.ts — validation + arg-shaping (no idem_key).
...
Deno.test('match-pass-offer: missing offer -> 400', async () => {
  setEnv();
  resetStub();
  assertEquals((await handler(post({}))).status, 400);
});

Deno.test('match-pass-offer: shapes p_actor/p_offer only', async () => {
  setEnv();
  const stub = resetStub({ user: { id: 'actor-1' } });
  await handler(post({ offer: 'o-1' }));
  assertEquals(stub.calls[0].name, 'match_pass_offer');
  assertEquals(stub.calls[0].args, { p_actor: 'actor-1', p_offer: 'o-1' });
});
