...
2. ✅ **Email magic-link rate-limited** → FIXED 2026-06-01: Supabase **Auth custom SMTP** enabled → Resend (`smtp.resend.com:465`, user `resend`, sender `hello@tryafter5.app`, sending-only key, per-user interval 10s). `tryafter5.app` was already DNS-verified in Resend (product emails delivering). Verified: a triggered magic-link send returned HTTP 200 (no SMTP error). Auth emails (login/OTP/confirm) no longer use the rate-limited built-in mailer.
