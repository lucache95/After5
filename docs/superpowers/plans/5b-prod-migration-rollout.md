...
| _(C-Edge — 8 Edge Functions; deployed via `supabase functions deploy`, not via `apply_migration`)_ | 2026-05-28T17:00Z | GREEN | **C-Edge** (deploys, not migrations): 8 Deno Edge Functions deployed ACTIVE on prod with `verify_jwt=true`. Each is a thin transport (~20 lines) wrapping the corresponding match_* RPC. Shared library: `_shared/match.ts` (withMatchHandler wrapper: CORS + JWT verify via supabase-js auth.getUser + body parse + errcode mapping + idem-key mint), `_shared/errcode.ts` (P5000-P5009 → HTTP status + UI string), `_shared/cors.ts` (pre-existing). Function list: match-shortlist (v2), match-make-offer, match-accept-offer, match-pass-offer, match-withdraw, match-cancel-lock, match-resolve-reciprocal, match-demand-hint. Smoke-tested: all reject no-Authorization with 401 (platform layer pre-handler); OPTIONS returns 200 for CORS preflight. End-to-end auth→RPC chain validated indirectly via A/B SQL tests against the same RPCs. |
| `20260527127100_p5_reciprocal_pair_wire` | applied 2026-05-29 | GREEN | 5b remediation: reciprocal Option (b). `match_make_offer(uuid,uuid,uuid,uuid)` dropped+recreated returning **jsonb** (was uuid); reciprocal now upserts `reciprocal_pairs` + dispatches both notifications + RETURNS `{kind:'reciprocal',pair_id}` instead of raising P5008. Verified post-apply: `pg_get_function_result` = `jsonb`; anon EXECUTE = false, authenticated = true. Flag `match_v2_enabled` OFF — safe. |
| `20260527127200_p5_job_rpcs_backfill` | applied 2026-05-29 | GREEN | R1 poison-loop fix: backfilled `match_bulk_withdraw(uuid)` + `close_rating_window(uuid)` (both SECURITY DEFINER, service-role-only, idempotent + stale-tolerant). Added `locks.rating_closed_at timestamptz`. Verified: both functions present (count=1 each), column present. |
| `20260527127300_p5_feature_config_read_policy` | applied 2026-05-29 | GREEN | Added `feature_config_public_read` SELECT policy (anon+authenticated, USING true) so SSR/client reads of non-secret flags (incl. `match_v2_enabled`, value=false) resolve instead of deny-all. Writes stay service/admin only. Verified: policy present, cmd=SELECT, roles {anon,authenticated}, qual=true. |
| `20260527127400_p5_host_pre_offer_disclosure` | applied 2026-05-29 | YELLOW (new WARN — see notes) | 5b-D fix: additive SELECT-only `profiles_select_host_queue` policy + DEFINER helper `match_host_can_see_candidate(uuid,uuid)` so a date creator can read pre-offer queued candidate profiles (interested/shortlisted/standby/offer_active). Verified: helper + policy present. **Advisor (new WARN, not ERROR):** `anon_security_definer_function_executable` on the new helper (default PUBLIC grant leaves anon EXECUTE on a bool-only DEFINER fn) + `multiple_permissive_policies`/`auth_rls_initplan` on profiles SELECT — all WARN; same accepted-risk class as A.7; flagged for follow-up, not fixed (migration applied as written). |
| `20260527127500_p5_offer_recipient_date_read` | applied 2026-05-29 | YELLOW (new WARN — see notes) | 5b-E fix: additive SELECT-only `date_instances_select_offer_recipient` policy + DEFINER helper `match_offer_recipient_can_see_instance(uuid,uuid)` so an offer recipient / lock participant can read the offered date_instance. Verified: helper + policy present. **Advisor (new WARN, not ERROR):** same `anon_security_definer_function_executable` on the new helper + `multiple_permissive_policies`/`auth_rls_initplan` on date_instances SELECT — all WARN; flagged for follow-up, not fixed. |
| `20260527127600_revoke_anon_execute` | applied 2026-05-29 | GREEN | Deploy-audit hardening: revoked anon EXECUTE on `record_swipe`, `post_night`, `browse_feed_for_viewer` (direct anon grant), and `match_reveal_allowed` (anon inherited via implicit PUBLIC grant — required `revoke ... from public` too; applied to prod as a follow-on statement so prod matches the file). Verified post-apply: all four anon EXECUTE = false, authenticated + service_role EXECUTE = true. |
| `20260527127700_p5_reveal_hardening` | applied 2026-05-30 | GREEN | 5b pre-launch reveal hardening (two fixes, flag still OFF). **#27** REVOKE public+anon EXECUTE on the two DEFINER reveal helpers `match_host_can_see_candidate(uuid,uuid)` (127400) + `match_offer_recipient_can_see_instance(uuid,uuid)` (127500), keeping authenticated (RLS policies invoke them in the querying role). `match_reveal_allowed`/`match_reveal_allowed_pair` already anon=false (stripped by 126650/127600) — no REVOKE needed, verified pre-apply. **#28** expiry-gate the active-offer branch in three predicates: `o.status in ('active','accepted')` → `(o.status = 'accepted' OR (o.status = 'active' AND o.expires_at > now()))` in `match_reveal_allowed` (1 spot), `match_reveal_allowed_pair` (2 spots, both directions), `match_offer_recipient_can_see_instance` (1 spot); 'accepted'/lock branches unchanged. Verified post-apply: all 4 fns anon=false/public=false/authenticated=true; the 3 predicate bodies contain `o.expires_at > now()` and no longer contain the old `in ('active','accepted')`. **Advisor (security):** the two `0028 anon_security_definer_function_executable` WARNs for the helpers are CLEARED (only `st_estimatedextent` PostGIS C fn remains under 0028); the helpers now appear only under `0029 authenticated_security_definer_function_executable`, the intended state (authenticated must retain EXECUTE for the RLS predicates). Local: new test `d_reveal_expiry.sql` (4 cases) + reveal regression suite + full `npm run db:test` (52 files) all exit 0. |

**Edge functions redeployed in this remediation (deploys, not migrations), 2026-05-29:**
- `match-make-offer` v2 → **v3** ACTIVE (verify_jwt=true) — picks up the jsonb-return contract; pass-through unchanged.
- `process-jobs` — was **ABSENT** on prod (every-minute cron 404'd); deployed **v1** ACTIVE (verify_jwt=false, service-to-service via `x-jobs-secret`). Includes the `bulk_withdraw` payload-key `'user'` fix.
- `match-resolve-reciprocal` v2 → **v3** ACTIVE (verify_jwt=true) — synced to current local source (testability refactor; behavior identical).
