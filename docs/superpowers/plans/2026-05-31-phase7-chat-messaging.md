...
> ## ✅ DECISIONS LOCKED (2026-06-01) — these OVERRIDE the defaults below
> The five open questions are now answered. Build to THIS, not to any conflicting default text further down:
> 1. **Chat timing → PRE-LOCK.** Chat opens on an open offer (Gate A as written: messageable while `state IN ('open','promoted') AND revoked_at IS NULL`). No change to the plan's default.
> 2. **Rapport-gate → SOFT NUDGE, NOT a hard gate.** **Task 5 is DROPPED.** Do **NOT** amend `chat_lock_ready`; do **NOT** create migrations `20260601100300` / `20260601100400`; do **NOT** add the `P5005 chat_not_ready` accept-block or its integration assertion. `match_accept_offer` stays exactly as shipped. `both_ready` is computed (Task 3, `chat_recompute_both_ready`) and used **only as a UI affordance** — surface a gentle nudge in the conversation/offer UI (e.g. "say hi before you lock in" until `both_ready`, then a subtle "you've both said hi 👋"). Locking is never blocked by chat.
> 3. **Rapport bar → 1 message each.** Keep `both_ready = each party has sent ≥1 message`, isolated in `chat_recompute_both_ready`. (Drives the nudge in #2, not a gate.)
> 4. **Threading → ONE THREAD PER OFFER/NIGHT.** Plan default. No collapsing per-counterpart.
> 5. **Moderation → INCLUDE a minimal report affordance (NEW TASK).** Add a minimal "report message" capability: (a) DB — a `message_reports` table (`id, message_id FK→messages, reporter_id, reason text null, created_at`), RLS deny-by-default with a SECURITY DEFINER `report_message(p_actor, p_message, p_reason)` RPC that asserts the actor is a party to the message's thread and is NOT the sender, inserts a report (idempotent per reporter+message), REVOKEd from public/anon; (b) UI — a small "report" action on a received message bubble in the conversation view (overflow/long-press → confirm → toast), Barbiecore-styled; (c) tests (party-only, not-own-message, idempotent + UI action). Minimal — no admin review-queue UI this round (reports land in the table; note follow-up in §8).
>
> Net effect on the task list: build Tasks 1–4 and 6–13 as written, **SKIP Task 5 entirely**, and ADD the report task (DB + RPC + UI + tests) — fold its migration into the band, its UI into the conversation-view task. Update §8 (moderation no longer fully out of scope) and drop risk **R1** (no shipped-flow change anymore).
