...
**Architecture:** Each route is a `force-dynamic` server component (5a `feed/page.tsx` pattern: `await createClient()` → `auth.getUser()` → gate → fetch Tier-3 → props to a `'use client'` child). Client mutations flow through one typed wrapper (`lib/after5/match.ts`) over `browserAfter5Client().functions.invoke('match-*')` that centralizes P5xxx→message mapping and idem_key minting; Realtime queue inserts arrive through `lib/after5/realtime.ts`, user-id-scoped.
...
| File | Responsibility |
| --- | --- |
| `apps/web/lib/after5/match.ts` | Typed wrapper over the `match-*` edge functions; `MatchError(code, detail)`; centralized P5000–P5009→message map; idem_key minting (`crypto.randomUUID`). |
...
| `apps/web/app/dates/[instanceId]/interested/MakeOfferModal.tsx` | Client: vaul sheet, expiry preview, confirm → `makeOffer`, errcode→toast, P5008→/reciprocal redirect. |
...
- [ ] Write the failing test:
  ...
  describe('match wrapper', () => {
    ...
    it('makeOffer mints an idem_key and returns the offer id', async () => {
      invoke.mockResolvedValue({ data: { ok: true, data: { offer_id: 'off-9' } }, error: null });
      const id = await makeOffer('inst-1', 'cand-1');
    ...
    it('throws MatchError carrying code + detail on a P-code body', async () => {
      invoke.mockResolvedValue({
        data: { ok: false, code: 'P5008', detail: { pair_id: 'pair-7' } },
        error: null,
      });
      await expect(makeOffer('inst-1', 'cand-1')).rejects.toMatchObject({
        code: 'P5008',
        detail: { pair_id: 'pair-7' },
      });
    });

    it('throws MatchError P5008 on a FunctionsHttpError envelope too', async () => {
      invoke.mockResolvedValue({
        data: { ok: false, code: 'P5008', detail: { pair_id: 'pair-3' } },
        error: { name: 'FunctionsHttpError', message: 'non-2xx' },
      });
      await expect(makeOffer('inst-1', 'cand-1')).rejects.toMatchObject({ code: 'P5008' });
    });

    it('messageForCode maps known codes to dry lowercase copy', () => {
      expect(messageForCode('P5003')).toMatch(/already have an offer out/i);
      expect(messageForCode('P5004')).toMatch(/overlaps/i);
      expect(messageForCode('P5002')).toMatch(/can.t be offered/i);
    });
  });
  ```
...
- [ ] Implement the wrapper:
  ...
  // edge function D touches; centralizes the P5000–P5009 → dry-copy mapping so
  ...
  export class MatchError extends Error {
    code: MatchCode | 'unknown';
    detail: unknown;
    constructor(code: MatchCode | 'unknown', detail?: unknown) {
  ...
  const MESSAGES: Record<MatchCode, string> = {
    P5000: 'matching isn’t live yet — soon.',
    P5001: 'that’s not your account to act on.',
    P5002: 'this person can’t be offered right now.',
    P5003: 'you already have an offer out.',
    P5004: 'that time overlaps another locked date.',
    P5007: 'that offer already expired.',
    P5008: 'this date needs a reciprocal decision first.',
    P5009: 'both dates were cancelled.',
  };

  export function messageForCode(code: string): string {
    return MESSAGES[code as MatchCode] ?? 'that didn’t go through. try again?';
  }

  type Envelope<T> = { ok: boolean; data?: T; code?: string; detail?: unknown };

  // Edge functions return a 2xx JSON envelope on the happy path AND, for handled
  // P-codes, may surface as a non-2xx with the same body on `data`. We read the
  // body either way and throw MatchError when ok === false.
  async function call<T>(fn: string, body: Record<string, unknown>): Promise<T> {
    ...
    if (data.ok === false) throw new MatchError((data.code as MatchCode) ?? 'unknown', data.detail);
  ...
  export async function makeOffer(instance: string, candidate: string): Promise<string> {
    const res = await call<{ offer_id: string }>('match-make-offer', {
...
- [ ] Write the failing test:
  ...
  describe('MakeOfferModal', () => {
    ...
    it('confirm calls makeOffer and reports success', async () => {
      makeOffer.mockResolvedValue('off-1');
    ...
    it('P5008 routes to the reciprocal screen', async () => {
      makeOffer.mockRejectedValue(new MatchError('P5008', { pair_id: 'pair-9' }));
      render(<MakeOfferModal open instanceId="inst-1" candidate={candidate} offerWindowHours={24} onOffered={vi.fn()} onClose={vi.fn()} />);
      await userEvent.click(screen.getByRole('button', { name: /send the offer/i }));
      await waitFor(() => expect(push).toHaveBeenCalledWith('/reciprocal/pair-9'));
    });

    it('P5003 toasts the already-active message', async () => {
      makeOffer.mockRejectedValue(new MatchError('P5003'));
      render(<MakeOfferModal open instanceId="inst-1" candidate={candidate} offerWindowHours={24} onOffered={vi.fn()} onClose={vi.fn()} />);
      await userEvent.click(screen.getByRole('button', { name: /send the offer/i }));
      await waitFor(() => expect(toastError).toHaveBeenCalledWith(expect.stringMatching(/already have an offer out/i)));
    });
  });
  ```
- [ ] Run it (expected FAIL):
...
- [ ] Implement:
  ...
  // in). Confirm → match.makeOffer (idem_key minted in the wrapper). Errors map
  // through messageForCode; P5008 redirects to the reciprocal chooser carrying
  // the pair_id from the error detail (DESIGN-SYSTEM §4 / spec §4.3).
  'use client';
  import { useState } from 'react';
  ...
  export function MakeOfferModal({
    ...
    async function confirm() {
      ...
      setBusy(true);
      try {
        await makeOffer(instanceId, candidate.candidate_id);
        toast.success(`offer’s out to ${candidate.first_name.toLowerCase()}.`);
        onOffered(candidate.candidate_id);
        onClose();
      } catch (e) {
        if (e instanceof MatchError && e.code === 'P5008') {
          const pairId = (e.detail as { pair_id?: string } | undefined)?.pair_id;
          if (pairId) {
            router.push(`/reciprocal/${pairId}`);
            return;
          }
        }
...
- [ ] Write the failing test:
  ...
  describe('ReciprocalChooser', () => {
    ...
    it('P5009 toasts both-cancelled and redirects home', async () => {
      resolveReciprocal.mockRejectedValue(new MatchError('P5009'));
      render(<ReciprocalChooser pairId="pair-1" instanceA={inst('a', 'jazz bar')} instanceB={inst('b', 'pottery')} />);
...
- [ ] Implement:
  ...
  // → redirect to the chosen instance's interested list. P5009 (stale) → toast +
  // home. Tier-2 experience surface chrome, dating-tone polaroids.
  'use client';
  import { useState } from 'react';
  import { useRouter } from 'next/navigation';
  import { toast } from 'sonner';
  ...
  export function ReciprocalChooser({
    ...
    async function pick(instance: ReciprocalInstance) {
      ...
      try {
        ...
        if (e instanceof MatchError && e.code === 'P5009') {
          toast.error(messageForCode('P5009'));
          router.push('/home');
          return;
        }
